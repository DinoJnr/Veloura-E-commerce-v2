// mails/refundrejectedorderpaymentmail.js
...
const refundRejectedOrderPaymentMail = async (order) => {
  const {
    customer,
    items = [],
    totalAmount,
    ...
    <div style="max-width:600px; margin:32px auto; background:#ffffff; border-radius:8px;
      ...
      <div style="background:#1a3a2a; padding:14px 32px; display:flex; align-items:center; gap:10px;">
        ...
        <span style="color:#9fe1cb; font-size:12px; letter-spacing:0.12em; text-transform:uppercase;">
          Refund initiated &amp; being processed
        </span>
      </div>
      ...
      <div style="padding:32px;">
        ...
        <p style="font-size:14px; color:#5a6a7a; line-height:1.7; margin:0 0 12px;">
          We sincerely apologise for the inconvenience. Unfortunately, your order
          <strong style="color:#0d1b2a;">#${orderId || "—"}</strong> could not be fulfilled due to
          the unavailability of one or more items, or unforeseen operational issues beyond our control.
        </p>
        <p style="font-size:14px; color:#5a6a7a; line-height:1.7; margin:0 0 24px;">
          Your payment has been processed for a <strong style="color:#1a4731;">full refund</strong>.
          Please allow <strong>3–7 business days</strong> for the funds to reflect, depending on
        ...
        <table style="width:100%; margin-bottom:24px;" cellpadding="0" cellspacing="8">
          <tr>
            ...
            <td style="background:#fff3f3; border-radius:4px; padding:8px 12px; width:25%;
              ...
              <span style="font-size:13px; font-weight:500; color:#922b21;">Refunded</span>
            </td>
            <td style="background:#f0f4f8; border-radius:4px; padding:8px 12px; width:25%;">
              <span style="font-size:10px; color:#7a8dae; text-transform:uppercase;
          ...
          <tfoot>
            <tr style="border-top:1.5px solid #0d1b2a;">
              <td colspan="3" style="padding:14px 12px; text-align:right; font-size:12px;
                                     ...
                                     letter-spacing:0.12em; font-weight:500;">Total Refund</td>
        ...
        <div style="background:#f8f9fa; border-radius:6px; padding:16px; margin-bottom:28px;
          ...
          <p style="font-size:13px; color:#1a4731; margin:0; line-height:1.75;">
            💚 <strong>Refund of ${formatCurrency(totalAmount)}</strong> is on its way to
            <strong>${customerEmail}</strong>. If you do not receive it within 7 business days,
            contact us quoting your order reference.
          </p>
        </div>
  ...
  // ── Send (same pattern as sendOrderConfirmationEmail) ──────────────────────
  return transporter.sendMail({
    from: `"Veloura Atelier" <${process.env.EMAIL_USER}>`,
    to: customerEmail,
    subject: `Refund Confirmed — #${orderId || "—"} | Veloura`,
    html,
  });
};

module.exports = { refundRejectedOrderPaymentMail };