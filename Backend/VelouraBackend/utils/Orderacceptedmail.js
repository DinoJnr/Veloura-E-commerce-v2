// mails/orderacceptedmail.js
// Trigger: Order status moves from  pending → accepted
...
const orderAcceptedMail = async (order) => {
  const {
    customer,
    items = [],
    totalAmount,
  ...
  <body style="margin:0; padding:0; background:#f0f2f5; font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
    <div style="max-width:600px; margin:32px auto; background:#ffffff; border-radius:8px;
      ...
      <!-- NOTICE BAR -->
      <div style="background:#1a2f4a; padding:14px 32px; display:flex; align-items:center; gap:10px;">
        ...
        <span style="color:#a8c4e0; font-size:12px; letter-spacing:0.12em; text-transform:uppercase;">
          Order confirmed &amp; now being prepared
        </span>
      </div>
      ...
      <div style="padding:32px;">
        ...
        <p style="font-size:14px; color:#5a6a7a; line-height:1.7; margin:0 0 12px;">
          Great news — your order <strong style="color:#0d1b2a;">#${orderId || "—"}</strong> has been
          <strong style="color:#1a3a5c;">accepted</strong> and is now being carefully processed by our team.
        </p>
        <p style="font-size:14px; color:#5a6a7a; line-height:1.7; margin:0 0 24px;">
          We are preparing your items with the attention to quality and detail that every Veloura order deserves.
          You will receive another update once your order is on its way.
        </p>
        ...
        <table style="width:100%; margin-bottom:24px;" cellpadding="0" cellspacing="8">
          <tr>
            ...
            <td style="background:#e8f0fb; border-radius:4px; padding:8px 12px; width:25%;
              ...
              <span style="font-size:13px; font-weight:500; color:#1a3a6e;">Processing</span>
            </td>
            <td style="background:#f0f4f8; border-radius:4px; padding:8px 12px; width:25%;">
              <span style="font-size:10px; color:#7a8dae; text-transform:uppercase;
        ...
        <div style="background:#f8f9fa; border-radius:6px; padding:16px; margin-bottom:28px;
          ...
          <p style="font-size:13px; color:#5a6a7a; margin:0; line-height:1.75;">
            Our team is now picking, quality-checking, and packaging your items. Once they are ready
            to leave our warehouse, we will send you a shipping notification with tracking details.
          </p>
        </div>
  ...
  return transporter.sendMail({
    ...
    to: customerEmail,
    subject: `Order Accepted — #${orderId || "—"} | Veloura`,
    html,
  });
};
