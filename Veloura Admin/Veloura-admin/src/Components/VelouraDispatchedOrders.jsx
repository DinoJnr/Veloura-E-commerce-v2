...
import {
  ...
  User, Package, CreditCard, CheckCircle2
} from "lucide-react";
        ...
        <div className="modal-drawer">
          ...
          <div className="modal-head">
            ...
            <div style={{ marginTop: "14px", display: "flex", gap: "8px" }}>
              <span style={{ background: "#e6f4ea", color: "#1e7e34", fontSize: "10px", fontWeight: 700, padding: "4px 10px", borderRadius: "6px", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                {order.paymentStatus || "—"}
              </span>
...
const ReceivedConfirmDialog = ({ orderIds, onConfirm, onCancel, loading }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(15,30,15,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "white", borderRadius: "24px", padding: "40px", width: "400px", textAlign: "center", boxShadow: "0 24px 80px rgba(0,0,0,0.14)" }}>
        <div style={{ width: "56px", height: "56px", borderRadius: "50%", background: "#e6f4ea", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
          <CheckCircle2 size={24} color="#1e7e34" />
        </div>
        <h3 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "22px", color: "#0d1f0f", margin: "0 0 10px" }}>
          Mark {isBulk ? `${orderIds.length} Orders` : "Order"} as Received?
        </h3>
        <p style={{ fontSize: "13px", color: "#88a088", margin: "0 0 28px", lineHeight: 1.6 }}>
          This will move the {isBulk ? "orders" : "order"} to <strong style={{ color: "#1e7e34" }}>Received</strong> and remove {isBulk ? "them" : "it"} from Dispatched. This cannot be undone.
        </p>
        <div style={{ display: "flex", gap: "12px" }}>
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            style={{ flex: 1, padding: "13px", borderRadius: "12px", border: "none", background: "#1e7e34", color: "white", cursor: loading ? "not-allowed" : "pointer", fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", opacity: loading ? 0.7 : 1 }}
          >
            {loading ? <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> : <CheckCircle2 size={14} />}
            {loading ? "Marking..." : "Yes, Mark as Received"}
          </button>
        </div>
      </div>
    </div>
...
const VelouraDispatchedOrders = () => {
  ...
  useEffect(() => { fetchDispatched(); }, []);

  const fetchDispatched = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5200/order/dispatched");
      setOrders(res.data.orders || res.data || []);
    } catch (err) {
      console.error("Failed to fetch dispatched orders:", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReceived = async (ids) => {
    try {
      setActionLoading(true);
      await axios.post("http://localhost:5200/order/received", { orderIds: ids });
      setOrders((prev) => prev.filter((o) => !ids.includes(o._id)));
      setSelectedIds([]);
      setConfirm(null);
    } catch (err) {
      console.error("Mark as received failed:", err.message);
      alert("Failed to mark order(s) as received. Please try again.");
    } finally {
  ...
  return (
    <>
      <style>{`
        ...
        .dispatched-main { flex: 1; padding: 40px; }

        .dispatched-table-card {
        ...
        .p-table tr.selected td { background: #f0faf2; }
        ...
        .btn-received { background: #e6f4ea; color: #1e7e34; }
        .btn-received:hover { background: #1e7e34 !important; color: white !important; }
        ...
        .global-received { background: #1e7e34; color: white; }
        .global-received:hover { background: #176229; }
        .global-received:disabled { opacity: 0.4; cursor: not-allowed; }
        ...
        .checkbox-custom:checked { background: #1e7e34; border-color: #1e7e34; }

        @keyframes spin { to { transform: rotate(360deg) } }
      `}</style>

      <div className="dispatched-layout">
        <aside className="dispatched-sidebar">
          <VelouraAdminNavbar currentPage="Dispatched Orders" />
        </aside>

        <main className="dispatched-main">
          ...
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "36px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              ...
              <button
                className="global-btn global-received"
                disabled={selectedIds.length === 0}
                onClick={() => setConfirm({ ids: selectedIds })}
              >
                <CheckCircle2 size={15} /> Mark as Received
              </button>
            </div>
          </header>

          <div className="dispatched-table-card">
            ...
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #f0f4f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ color: "#1e7e34", display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: 500 }}>
                <CheckCircle2 size={15} />
                {filtered.length} order{filtered.length !== 1 ? "s" : ""} dispatched
              </div>
            </div>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="7" style={{ textAlign: "center", padding: "56px", color: "#88a088" }}>
                      {searchTerm ? `No orders matching "${searchTerm}"` : "No dispatched orders — all received! ✅"}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {confirm && (
        <ReceivedConfirmDialog
          ...
          onConfirm={() => handleReceived(confirm.ids)}
        />
      )}
    </>
  );
...
export default VelouraDispatchedOrders;