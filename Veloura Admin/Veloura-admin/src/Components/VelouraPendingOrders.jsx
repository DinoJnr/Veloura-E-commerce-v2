...
import {
  ...
  User, Package, CreditCard, Hash, MapPin
} from "lucide-react";
...
// ─── CONFIRM DIALOG ───────────────────────────────────────────────────────────
const ConfirmDialog = ({ type, orderIds, onConfirm, onCancel, loading }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(15,30,15,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "white", borderRadius: "24px", padding: "40px", width: "400px", textAlign: "center", boxShadow: "0 24px 80px rgba(0,0,0,0.14)", animation: "overlayFadeIn 0.2s ease" }}>
        <div style={{ width: "56px", height: "56px", borderRadius: "50%", background: isAccept ? "#eaf3de" : "#fcebeb", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
          {isAccept ? <Check size={24} color="#3b6d11" /> : <X size={24} color="#a32d2d" />}
        </div>
        <h3 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "22px", color: "#0d1f0f", margin: "0 0 10px" }}>
          {isAccept ? "Accept" : "Reject"} {isBulk ? `${orderIds.length} Orders` : "Order"}?
        </h3>
        <p style={{ fontSize: "13px", color: "#88a088", margin: "0 0 28px", lineHeight: 1.6 }}>
          {isAccept
            ? "This will move the order(s) to Processed and remove from Pending."
            : "This will permanently remove the order(s) from Pending. This cannot be undone."}
        </p>
        <div style={{ display: "flex", gap: "12px" }}>
          <button onClick={onCancel} style={{ flex: 1, padding: "13px", borderRadius: "12px", border: "1px solid #e8ede8", background: "white", cursor: "pointer", fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 600, color: "#88a088" }}>
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={loading}
            style={{ flex: 1, padding: "13px", borderRadius: "12px", border: "none", background: isAccept ? "#2e4a2e" : "#a32d2d", color: "white", cursor: loading ? "not-allowed" : "pointer", fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", opacity: loading ? 0.7 : 1 }}
          >
...
const VelouraPendingOrders = () => {
  ...
  const [confirm, setConfirm] = useState(null); // { type: "accept"|"reject", ids: [] }
  ...
  const fetchPending = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5200/order/pending");
  ...
  const handleAccept = async (ids) => {
    try {
      ...
      await axios.post("http://localhost:5200/order/accept", { orderIds: ids });
      setOrders((prev) => prev.filter((o) => !ids.includes(o._id)));
      setSelectedIds([]);
      setConfirm(null);
    } catch (err) {
      console.error("Accept failed:", err.message);
      alert("Failed to accept order(s). Please try again.");
    } finally {
  ...
  const handleReject = async (ids) => {
    try {
      setActionLoading(true);
      await axios.post("http://localhost:5200/order/reject", { orderIds: ids });
      setOrders((prev) => prev.filter((o) => !ids.includes(o._id)));
      setSelectedIds([]);
      setConfirm(null);
    } catch (err) {
      console.error("Reject failed:", err.message);
      alert("Failed to reject order(s). Please try again.");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
    <>
      <style>{`
        ...
        .pending-main { flex: 1; padding: 40px; }
        ...
        .p-table tr.selected td { background: #f0f8f0; }
        ...
        .btn-accept { background: #eaf3de; color: #3b6d11; }
        ...
        .global-accept { background: #2e4a2e; color: #f1f8e9; }
        .global-accept:hover { background: #1a3d1a; }
        .global-accept:disabled { opacity: 0.4; cursor: not-allowed; }
        .global-reject { background: white; color: #a32d2d; border: 1px solid #fcebeb !important; }
        .global-reject:hover { background: #fcebeb; }
        .global-reject:disabled { opacity: 0.4; cursor: not-allowed; }
        ...
        .checkbox-custom:checked { background: #2e4a2e; border-color: #2e4a2e; }
      ...
      <div className="pending-layout">
        <aside className="pending-sidebar">
          <VelouraAdminNavbar currentPage="Pending Orders" />
        </aside>
          ...
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "36px" }}>
            <div>
              ...
              <p style={{ color: "#88a088", fontSize: "13px", margin: "4px 0 0" }}>
                {orders.length} order{orders.length !== 1 ? "s" : ""} awaiting action
              </p>
            </div>
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              ...
              <button
                className="global-btn global-reject"
              ...
              <button
                className="global-btn global-accept"
                disabled={selectedIds.length === 0}
                onClick={() => setConfirm({ type: "accept", ids: selectedIds })}
              >
                <Check size={15} /> Accept Selected
              </button>
            </div>
          </header>
          ...
          <div className="pending-table-card">
            ...
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #f0f4f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ color: "#d97706", display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: 500 }}>
                <AlertCircle size={15} />
                {filtered.length} order{filtered.length !== 1 ? "s" : ""} need attention
              </div>
            </div>
                ...
                {loading ? (
                      ...
                      <td>
                        <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
                          ...
                          <button
                            ...
                            title="Accept Order"
                            onClick={() => setConfirm({ type: "accept", ids: [order._id] })}
                          >
                            <Check size={15} />
                          </button>
                          {/* REJECT */}
                  ...
                  <tr>
                    <td colSpan="7" style={{ textAlign: "center", padding: "56px", color: "#88a088" }}>
                      {searchTerm ? `No orders matching "${searchTerm}"` : "No pending orders — all clear! ✅"}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {confirm && (
        <ConfirmDialog
          ...
          onConfirm={() =>
            confirm.type === "accept"
              ? handleAccept(confirm.ids)
              : handleReject(confirm.ids)
          }
        />
      )}
    </>
...
export default VelouraPendingOrders;