...
import {
  Search, Eye, Truck, AlertCircle,
  Loader2, User, Package, CreditCard, X
} from "lucide-react";
...
// ─── TRANSIT CONFIRM DIALOG ───────────────────────────────────────────────────
const TransitConfirmDialog = ({ orderIds, onConfirm, onCancel, loading }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(15,30,15,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "white", borderRadius: "24px", padding: "40px", width: "400px", textAlign: "center", boxShadow: "0 24px 80px rgba(0,0,0,0.14)" }}>
        <div style={{ width: "56px", height: "56px", borderRadius: "50%", background: "#eef2ff", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
          <Truck size={24} color="#4f46e5" />
        </div>
        <h3 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "22px", color: "#0d1f0f", margin: "0 0 10px" }}>
          Send {isBulk ? `${orderIds.length} Orders` : "Order"} to Transit?
        </h3>
        <p style={{ fontSize: "13px", color: "#88a088", margin: "0 0 28px", lineHeight: 1.6 }}>
          This will move the {isBulk ? "orders" : "order"} to <strong style={{ color: "#4f46e5" }}>Transit</strong> and remove {isBulk ? "them" : "it"} from Processing. This cannot be undone.
        </p>
        <div style={{ display: "flex", gap: "12px" }}>
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            style={{ flex: 1, padding: "13px", borderRadius: "12px", border: "none", background: "#4f46e5", color: "white", cursor: loading ? "not-allowed" : "pointer", fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", opacity: loading ? 0.7 : 1 }}
          >
            {loading ? <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> : <Truck size={14} />}
            {loading ? "Moving..." : "Yes, Send to Transit"}
          </button>
        </div>
      </div>
    </div>
...
const VelouraProcessedOrders = () => {
  ...
  useEffect(() => { fetchProcessed(); }, []);

  const fetchProcessed = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://localhost:5200/order/processed");
      setOrders(res.data.orders || res.data || []);
    } catch (err) {
      console.error("Failed to fetch processed orders:", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMoveToTransit = async (ids) => {
    try {
      setActionLoading(true);
      await axios.post("http://localhost:5200/order/transit", { orderIds: ids });
      setOrders((prev) => prev.filter((o) => !ids.includes(o._id)));
      setSelectedIds([]);
      setConfirm(null);
    } catch (err) {
      console.error("Transit failed:", err.message);
      alert("Failed to move order(s) to transit. Please try again.");
    } finally {
  ...
  return (
    <>
      <style>{`
        ...
        .processed-main { flex: 1; padding: 40px; }
        ...
        .btn-transit { background: #eef2ff; color: #4f46e5; }
        ...
        .global-transit { background: #4f46e5; color: white; }
        .global-transit:hover { background: #3730a3; }
        .global-transit:disabled { opacity: 0.4; cursor: not-allowed; }
        ...
        .checkbox-custom:checked { background: #4f46e5; border-color: #4f46e5; }

        @keyframes spin { to { transform: rotate(360deg) } }
      `}</style>

      <div className="processed-layout">
        <aside className="processed-sidebar">
          <VelouraAdminNavbar currentPage="Processed Orders" />
        </aside>

        <main className="processed-main">
          ...
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "36px" }}>
            <div>
              ...
              <p style={{ color: "#88a088", fontSize: "13px", margin: "4px 0 0" }}>
                {orders.length} order{orders.length !== 1 ? "s" : ""} ready for dispatch
              </p>
            </div>
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              ...
              <button
                className="global-btn global-transit"
                disabled={selectedIds.length === 0}
                onClick={() => setConfirm({ ids: selectedIds })}
              >
                <Truck size={15} /> Send to Transit
              </button>
            </div>
          </header>
          ...
          <div className="processed-table-card">
            ...
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #f0f4f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ color: "#4f46e5", display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: 500 }}>
                <Truck size={15} />
                {filtered.length} order{filtered.length !== 1 ? "s" : ""} ready for transit
              </div>
            </div>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="7" style={{ textAlign: "center", padding: "56px", color: "#88a088" }}>
                      {searchTerm ? `No orders matching "${searchTerm}"` : "No processed orders — all dispatched! 🚚"}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {confirm && (
        <TransitConfirmDialog
          ...
          onConfirm={() => handleMoveToTransit(confirm.ids)}
        />
      )}
    </>
  );
...
export default VelouraProcessedOrders;