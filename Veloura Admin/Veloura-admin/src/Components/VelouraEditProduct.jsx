...
// 1. Added useLocation to the imports to receive your 'state'
...
const VelouraEditProduct = () => {
  ...
  const [form, setForm] = useState({
    name: "", brand: "", productId: "", price: "",
    comparePrice: "", stock: "", weight: "",
    origin: ""
  });
  ...
  const mapData = (p) => {
    ...
    const catObj = CATEGORIES.find(c => c.label === p.category);
    setActiveCat(catObj ? catObj.id : (p.category?.toLowerCase() || ""));
  ...
  useEffect(() => {
    const getProduct = async () => {
      ...
      if (location.state && location.state.product) {
        mapData(location.state.product);
        setLoading(false);
      } 
  
      else {
        try {
          const res = await axios.get(`http://localhost:5200/upload/getproduct/${id}`);
          if (res.data.success) {
            mapData(res.data.product);
          }
        } catch (err) {
  ...
  const cfg = CATEGORIES.find(c => c.id === activeCat) || CATEGORIES[0];
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.put(`http://localhost:5200/upload/updateproduct/${id}`, payload);
        ...
        <div className="max-w-7xl mx-auto px-5 py-10">
          <motion.div {...fadeUp(0)} className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-zinc-500 text-sm mt-1">Editing SKU: <span className="text-zinc-900 font-medium">{form.productId}</span></p>
            </div>
            <div className="flex gap-3">
              <button 
                onClick={() => navigate(-1)} 
                className="px-5 py-2.5 rounded-xl border border-zinc-200 bg-white text-xs font-bold text-zinc-400 hover:text-zinc-600"
              >
                Cancel
              </button>
              <button onClick={handleSubmit} className="px-5 py-2.5 rounded-xl text-xs font-bold text-white transition-all shadow-lg" style={{ backgroundColor: accent }}>
                {submitted ? "Saving Changes..." : "Update Product"}
              </button>
            </div>
          </motion.div>
            ...
            {activeCat && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 flex flex-col gap-6">
                  {/* Gallery */}
                  <Card delay={0.02}>
                    <CardHeader icon={Camera} title="Product Gallery" accent={accent} />
                    <div className="p-6">
                      <div className="grid grid-cols-4 gap-4">
                        ...
                        {images.length < 4 && (
                          <button onClick={() => fileRef.current.click()} className="aspect-square rounded-xl border-2 border-dashed border-zinc-100 flex flex-col items-center justify-center gap-2 hover:bg-zinc-50 transition-colors">
                            <Upload size={20} className="text-zinc-300" />
                            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Update Photo</span>
                          </button>
                        )}
                  ...
                  <Card delay={0.05}>
                    ...
                    <div className="p-6 grid gap-5">
                      <TextInput label="Product Name" required accent={accent} value={form.name} onChange={e => setField("name", e.target.value)} />
                      <div className="grid grid-cols-2 gap-4">
                        <TextInput label="Brand" accent={accent} value={form.brand} onChange={e => setField("brand", e.target.value)} />
                        <TextInput label="Stock Level" type="number" accent={accent} value={form.stock} onChange={e => setField("stock", e.target.value)} />
                      </div>
                    </div>
                  </Card>
                  ...
                  <Card delay={0.11}>
                    ...
                    <div className="p-6 flex flex-col gap-5">
                      <Dropdown label="Subcategory" required accent={accent} options={cfg.subcategories} value={subcategory} onChange={setSubcategory} />
                      ...
                      <ChipSelector label={cfg.sizeLabel} items={cfg.sizes} selected={sizes} onToggle={toggle(sizes, setSizes)} accent={accent} />
                        ...
                        <div className="border-t border-zinc-100 pt-5">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {cfg.extraFields.map(field => (
                              <TextInput key={field.key} label={field.label} accent={accent} value={extraFields[field.key] || ""} onChange={e => setExtraFields(p => ({ ...p, [field.key]: e.target.value }))} />
                            ))}
                          </div>
                        </div>
                      )}
                  ...
                  <Card delay={0.14}>
                    ...
                    <div className="p-6 grid grid-cols-2 gap-4">
                      <TextInput label="Country of Origin" accent={accent} value={form.origin} onChange={e => setField("origin", e.target.value)} />
                  ...
                  {/* Tags */}
                  <Card delay={0.16}>
                    <CardHeader icon={Hash} title="Tags & SEO" accent={accent} />
                    <div className="p-6">
                      <TextInput label="Add Tags" hint="Press Enter" accent={accent} value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={addTag} />
                      <div className="flex flex-wrap gap-2 mt-3">
                        {tags.map(tag => (
                          <span key={tag} className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-medium" style={{ backgroundColor: accent + "12", borderColor: accent + "30", color: accent }}>
                            #{tag}
                ...
                <div className="flex flex-col gap-5">
                  <Card delay={0.06}>
                    ...
                    <div className="p-6 flex flex-col gap-4">
                      ...
                      {discountPct > 0 && (
                        <div className="rounded-xl px-4 py-3 text-xs border" style={{ backgroundColor: "#f0fdf4", borderColor: "#bbf7d0", color: "#15803d" }}>
                          <span className="font-bold">{discountPct}% Discount</span> currently active.
                        </div>
                      )}
                    </div>
                  </Card>
                  ...
                  <Card delay={0.09}>
                    ...
                    <div className="p-6 flex flex-col gap-2.5">
                      {[{ val: "active", label: "Active", sub: "Live on store" }, { val: "draft", label: "Draft", sub: "Hidden" }].map(({ val, label, sub }) => (
                  ...
                  {/* Summary Sidebar */}
                  <motion.div {...fadeUp(0.15)} className="rounded-2xl p-6 text-white shadow-xl" style={{ background: `linear-gradient(135deg, #18181b, #27272a)` }}>
                    <div className="flex items-center gap-2 mb-5">
                      <span className="text-lg">{cfg.emoji}</span>
                      <p className="text-[10px] font-bold text-zinc-400 tracking-widest uppercase">Summary Overview</p>
                    </div>
                    <div className="space-y-3 text-xs">
                      {[
                        { k: "Product Name", v: form.name || "—" },
                        { k: "Selling Price", v: form.price ? `₦${parseFloat(form.price).toLocaleString()}` : "—", a: true },
                        { k: "Stock", v: `${form.stock || 0} units` },
                        { k: "Category", v: subcategory || cfg.label },
                      ].map(({ k, v, a }) => (
                        <div key={k} className="flex justify-between border-b border-zinc-700/50 pb-2 last:border-0">
                          <span className="text-zinc-500">{k}</span>
                          <span className="font-semibold text-right max-w-[140px] truncate" style={a ? { color: cfg.accent } : {}}>{v}</span>
                        </div>
                      ))}
                    </div>
                    <motion.button onClick={handleSubmit} whileTap={{ scale: 0.97 }} className="mt-6 w-full py-3.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2" style={{ backgroundColor: accent, color: "#fff" }}>
...
export default VelouraEditProduct;